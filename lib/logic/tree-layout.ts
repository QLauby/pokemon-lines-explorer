...
export function recalculateTreeLayout(nodes: TreeNode[]): TreeNode[] {
  ...
  function assignCoordinatesAndIndices(node: TreeNode, y: number, inheritedBranchIndex: number, parentCumulativeProb: number): number {
    ...
    // Root (no parent) always has a local probability of 1.0 (100%)
    let nodeProb = (typeof node.probability === "number" && !isNaN(node.probability)) ? node.probability : 1
    if (!node.parentId) nodeProb = 1
